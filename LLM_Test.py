...
# 创建结果文件夹
# 创建结果文件夹和模型的子目录
base_output_dir = "result_LLM"
# model_output_dir = os.path.join(base_output_dir, model_name)
# if not os.path.exists(model_output_dir):
#     os.makedirs(model_output_dir)

case_prompt = """
I say :I am going to give you a series of natural language inference tasks. Based on the evidence I provide (note: the evidence may contain errors), please determine if the claim should be labeled as “SUPPORT” ,“REFUTE” or “NOT ENOUGH INFO”.Please answer with only one of these options.Don't answer anything else!”
...
def generate_prompt(claim, evidences, evi_labels):
    ...
    prompt += "\nBased on the evidences above(note: the evidence may contain errors,you need to determine which ones are correct), does the claim 'REFUTE' , 'SUPPORT' or “NOT ENOUGH INFO”?,Please answer with only one of these options.Don't answer anything else!"

    prompt = case_prompt + prompt + "You just need to answer:'REFUTE' , 'SUPPORT' or “NOT ENOUGH INFO”,Please answer with only one of these options.Don't answer anything else!"
    return prompt
...
def process_all_files(json_files, model_names):
    for model_name in model_names:
        ...
        # 创建模型的结果文件夹
        model_output_dir = os.path.join("result_LLM", model_name)
        if not os.path.exists(model_output_dir):
            os.makedirs(model_output_dir)
