...
from utils import init_logger, load_pair_data, cal_nwgm_center_attention_LLM
from data_load_utils import build_dataset_LLM
from models import Walk_with_LLM
...
parser.add_argument("--evi_num", type=int, default=1, help='Evidence num.')
...
torch.use_deterministic_algorithms(False)
dir_path = Path('llama_outputs/fever_2way/')
...
train_data = build_dataset_LLM(train_data_path, args.evi_num, 16)
...
for path in dev_file_paths:
    dev_data = build_dataset_LLM(path, args.evi_num, 4)
    ...
    dev_centers_list[path] = cal_nwgm_center_attention_LLM(graph_rep_model_path, dev_dataloader, nclass, device,
                                                       feature_num, args.evi_num, n_cluster)

feature_num = 768
model = Walk_with_LLM(feature_num, nclass, max_length, beam_size, args.evi_num, causal_method)
...
train_centers = cal_nwgm_center_attention_LLM(graph_rep_model_path, train_tqdm_iterator, nclass, device, feature_num,
                                          args.evi_num, n_cluster)
step = 0
for epoch in range(args.epochs):
    ...
    for index, data in enumerate(train_tqdm_iterator):
        data = [i.to(device) for i in data]
        input_ids, input_mask, segment_ids, labels, sent_labels, evi_labels, cos = data  # sent_labels [batch,6] evi_labels [batch,6]
        ...
        description = 'epoch %d Acc: %lf, Loss: %lf' % (
        epoch, correct_pred / (index + 1) / args.batch_size, running_loss / (index + 1))
